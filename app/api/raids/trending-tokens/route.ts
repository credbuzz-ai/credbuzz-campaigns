...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const url = `${EXTERNAL_API_BASE}/raids/trending-tokens${queryString ? `?${queryString}` : ''}`
    ...
    return NextResponse.json(
      { error: 'Failed to fetch trending tokens', details: error instanceof Error ? error.message : 'Unknown error' }, 
      { status: 500 }
    )
  }
} 